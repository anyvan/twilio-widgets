...
interface ParticipantWithWorkerAttributes extends Participant {
    workerAttributes?: {
        ...
        workerEmail: string;
        workerAttributes: Record<string, unknown>;
        workspaceSid: string;
    };
}
...
export const useServiceRating = (): UseServiceRatingReturn => {
    ...
    const submitRating = useCallback(
        async (rating: number, feedback?: string): Promise<void> => {
            if (!conversationSid) {
            ...
            try {
                 ...
                 */
                const localParticipantIdentity = (conversation as { localParticipant?: { identity?: string } })
